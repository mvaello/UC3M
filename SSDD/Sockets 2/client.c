...
void f_ping(){
	...
	char bytes[1];
	ssize_t bytes_read;
	if((bytes_read = read(sockfd, bytes, 1)) == -1) {
...
void f_swap(char *src, char *dst){
	...
	char bytes[sizeof(struct swap_iter)];
	unsigned int i;
	for(i = 0; i < iter; i++) {
		//Iteración SWAP.
		...
		if((bytes_read = read(sockfd, bytes, sizeof(struct swap_iter))) == -1) {
...
void f_hash(char *src){
	...
	for(i = 0; i < iter; i++) {
		...
		//Esperamos la respuesta del servidor.
		if((bytes_read = read(sockfd, bytes, sizeof(struct hash_iter_res))) == -1) {
			perror("[ERROR] Fallo al recibir datos del servidor.\n");
			break;
		}
		else if(bytes_read == 0) {
...
void f_check(char *src, int hash){
	...
	for(i = 0; i < iter; i++) {
		...
		//Esperamos la respuesta del servidor.
		if((bytes_read = read(sockfd, bytes, sizeof(struct check_iter_res))) == -1) {
			perror("[ERROR] Fallo al recibir datos del servidor.\n");
			break;
		}
		else if(bytes_read == 0) {
...
void f_stat(){
	...
	char bytes[sizeof(struct client_stat)];
	ssize_t bytes_read;
	if((bytes_read = read(sockfd, bytes, sizeof(struct client_stat))) == -1) {
		perror("[ERROR] Fallo al recibir datos del servidor.\n");
		return;
	}
	else if(bytes_read == 0) {
